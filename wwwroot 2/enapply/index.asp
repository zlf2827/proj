...
Sub echoContent()
	...
	templatePath=sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&setting.htmlFilePath&"/enapply.html"
	if not CheckTemplateFile(templatePath) then echo "enapply.html"&err_16
...
Sub addApply
	...
	if isnul(Jobs) then alertMsgAndGo "The Position is not null","-1"
	if isnul(Linkman) then alertMsgAndGo "The Name is not null","-1"
	if isnul(Age) then alertMsgAndGo "The Age is not null","-1"
	if isnul(phone) then alertMsgAndGo "The Phone is not null","-1"
	...
	alertMsgAndGo "Successful application！",sitepath&setting.languagePath
End Sub
%>
