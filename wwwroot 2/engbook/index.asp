...
if isnul(request.QueryString) then 
	set rs=conn.exec("select SortID from {prefix}Sort where SortUrl='/engbook/'","r1")
...
Sub echoGbook(SortID, Page)
	...
	templatePath=sitePath&"/"&"templates/"&setting.defaultTemplate&"/"&setting.htmlFilePath&"/engbook.html"
	if not CheckTemplateFile(templatePath) then echo "engbook.html"&err_16
