...
Sub echoErr(byval str,byval id, byval des)	
	...
	errstr=cssstr&"<div id='msg'><div class='msgtitle'>提示：【"&str&"】</div><div id='msgbody'>错误号："&id&"<br>错误描述："&des&"</div><div id='msgbottom'></div></div>" 
	cssstr=""
	die(errstr)
End Sub
...
Function pageNumberLinkInfo(Byval currentPage,Byval pageListLen,Byval totalPages,Byval linkType,Byval sortid, Byval showType)
	...
	pageNumberInfo="<span> "&str_06&""&totalPages&" "&str_07&" "&str_05&":"&currentPage&"/"&totalPages&" "&str_07&"</span>"&firstPageLink&lastPagelink&pageNumber&""&nextPagelink&""&finalPagelink
...
Function getSubSort(sortID, allsub)
	dim rs, sql
	sql="select (select count(*) from {prefix}Sort where ParentID="&sortID&"), * from {prefix}Sort where ParentID="&sortID
